-- Fix signup trigger enum resolution when Supabase Auth runs with a restricted search_path.
-- Keep the current trigger logic unchanged; only qualify subscription_status and set search_path.
